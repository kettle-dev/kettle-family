...
RSpec.describe Kettle::Family::CLI do
  ...
  it "plans templating for only members matching release-state status tokens" do
    ...
    status = described_class.call(["template", "--root", @tmpdir, "--only", "unreleased"], out: out, err: StringIO.new)

    expect(status).to eq(0)
    expect(out.string).to include("* alpha")
    expect(out.string).to include("- beta")
    expect(out.string).to include("- gamma")
    expect(out.string).to include("skipped alpha prepare_template_dependencies")
    expect(out.string).not_to include("skipped beta prepare_template_dependencies")
    expect(out.string).not_to include("skipped gamma prepare_template_dependencies")
  end
  ...
  it "passes GitHub Actions SHA pin check and upgrade options through the CLI" do
    ...
    expect(result.fetch("command")).to eq(["sh", "-lc", "bundle exec kettle-gha-pins --check --upgrade minor"])
  end
  ...
  it "passes release resume options through the CLI" do
    ...
    status = described_class.call(
      ["release", "--root", @tmpdir, "--only", "alpha", "--publish", "--start-step", "10", "--skip-steps", "10", "--ci-workflows", "current,style.yml", "--local-ci", "--continue-ci-failures", "--skip-bundle-audit", "--skip-remotes", "cb", "--required-remotes", "origin", "--json"],
      out: out,
      err: StringIO.new
    )
