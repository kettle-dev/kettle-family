...
RSpec.describe Kettle::Family::Workflow do
  ...
  it "preserves local dependency switches while excluding changelog tooling during nomono bootstrap" do
    write_template_config(
      command: ["bundle", "exec", "kettle-jem", "install"],
      normalize_lockfiles: false,
      family_mode: "sibling_repos"
  ...
  it "restores lockfiles generated by a branch test before the next target checkout" do
    write_template_config(release_target_branches: %w[r1 r2])
    config = Kettle::Family::Config.load(root: @tmpdir)
    member = member_at("alpha")
    ...
    allow(Kettle::Family::GitStatus).to receive(:dirty_paths).with(member.root).and_return(
      [],
      [" M Gemfile.lock"],
      []
    )
    ...
    expect(results.map(&:phase)).to eq([
      "release_checkout", "test", "branch_generated_lockfile_recovery",
      "release_checkout", "test"
    ])
    recovery = calls.find { |call| call[:phase] == "branch_generated_lockfile_recovery" }
